...
wrf.list = [w for w in wrf if w.name=="WRF09.20130828.2100"]
...
for thres in [15, 25, 30]:
    ...
    for sigma in [60, 40, 20]:
        ...
        candidates  = correlations[: len(correlations)//2+1]  # taking the top half going down the scale
...
open('gaussianSmoothNormalisedCorrelationTest.log' + timeString + '.txt', 'a').write('\n\n\n====\nAll Correlations'+\
                                                                    str(correlationsAll))
