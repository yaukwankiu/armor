...
#   to do
#   1. rewrite the interpolation stuff
#   2. redo the coordinate computations
...
reload(test); x = test.main() ; print "x['test3_result']['correlation']=", x['test3_result']['correlation']
...
def showArrayWithAxes(arr, cmap='jet', display=True, outputPath=""):
...
def interpolate(arr, I, J, useRBS=True):
    ...
    if useRBS:
        #   new loop 2013-10-03
        #   ref:  armor/geometry/regrid.py
        height_arr, width_arr   = arr.shape
        arr_spline  = rbs(range(height_arr), range(width_arr), arr)
        for i in range(height):
            for j in range(width):
                arr2[i,j]   = arr_spline(I[i,j],J[i,j])
    else:
...
def getCorrelation(arr1, arr2):
    ...
    return ma.corrcoef(arr1.reshape(arr1.size), arr2.reshape(arr2.size))[0,1]
