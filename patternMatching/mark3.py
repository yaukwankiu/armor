...
x=mark3.main(verbose=True, saveImage=False, display=False)  #<-- change it to saveImage=False to save space
...
def getWrfFolders(wepsFolder=wepsFolder, a="", maxTimeDiff=6., reportLength=72):
...
def getWrfFrameIndices(a=a, wrfFolder=wrfFolder, maxTimeDiff=6, timeInterval=3):
    #   2014-06-01
...
def main(radarPath=radarPath, wepsFolder=wepsFolder, **kwargs):
    a   = getRadarDBZ(radarPath)
    print "\n==============================================================="
    print "comparing", a.name, a.dataTime
    print "to", wepsFolder
    scores = matching(a, wepsFolder, **kwargs)
