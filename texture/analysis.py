...
def computeSegmentation(clust,outputFolder = textureThickFolder):
    ...
    for i in range(k):
        ...
        pic = dbz(  name='texture layer %d thickened' %i,
...
def main(a=a, k=k, scales=scales, NumberOfOrientations = NumberOfOrientations, featureFolder="",
           textureFolder="", textureFolderThick="", rootFolder="", doSegmentation=True,
           memoryProblem=True):
    ...
    if textureFolderThick=="":
        textureThickFolder = "armor/texture/%d/textureLayers%dthick/" % (timestamp, k)
    if rootFolder =="":
        rootFolder = "armor/texture/%d/" %timestamp
    ##################################################
    ...
    return_value = {'texturelayer': texturelayer, 
                    ...
                    'clust'       : clust,
                    'a'           : a,}
    pickle.dump(return_value, open(textureFolder+ 'texturelayes_and_segmentations.pydump','w'))
    # end:  save the original image for the record
    a.imagePath=rootFolder + 'a'
    a.flipud().saveImage()
    try:
